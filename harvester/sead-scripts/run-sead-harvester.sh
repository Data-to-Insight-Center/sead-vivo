...
export HARVESTER_INSTALL_DIR=/home/isuru/software/vivo/harvester/
export HARVEST_NAME=sead
...
#export CLASSPATH=$CLASSPATH:$HARVESTER_INSTALL_DIR/bin/config:$HARVESTER_INSTALL_DIR/bin/harvester.jar:$HARVESTER_INSTALL_DIR/bin/sead-harvester-1.0.jar:$HARVESTER_INSTALL_DIR/bin/dependency/*
export CLASSPATH=$CLASSPATH:$HARVESTER_INSTALL_DIR/bin/harvester.jar:$HARVESTER_INSTALL_DIR/bin/sead-harvester-1.0.jar:$HARVESTER_INSTALL_DIR/bin/dependency/*
...
harvester-seadhttpfetch -X seadfetch.config.xml
...
harvester-seadxsltranslator -X xsltranslator.config.xml
...
# 	is created with the values / scores of the data comparisons.
# We execute scores in 2 different steps, known as "tiered scoring". The initial score limits our input set to speed up performance
harvester-score -X score-author-family-name.config.xml
...
harvester-changenamespace -X changenamespace-work.webpage.config.xml
...
#Output some counts
#PROFILES=`cat data/vivo-additions.rdf.xml | grep ':path' | wc -l`
PROFILES=`ls data/raw-records | wc -l`
PUBLICATIONS=`cat data/vivo-additions.rdf.xml | grep 'work-id' | wc -l`
#echo "Imported 1 profile(s) with $PUBLICATIONS publication(s)"
echo "Imported $PROFILES profile(s) with $PUBLICATIONS publication(s)"
